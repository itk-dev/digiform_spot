...
function create_li_element (key, value) {
  ...
  return '<li><img id="' + myConfig.id_prefix_images + key + '" data-id="' + value + '" src="' + spotdata.prefix + spotdata.id[value].src + '" width="' + myConfig.max_image_width + '" height="' + myConfig.max_image_height + '" alt="" /></li>'
}

function update_li_element(key, value){
  // opdaterer li element i banneret
  $('#' + myConfig.id_prefix_images + key).attr('src', spotdata.prefix + spotdata.id[value].src ).data('id', value);
}
...
function show_popupbox(isbn) {
  ...
  //
  if (spotdata.id[isbn].d == null) {
    spotdata.id[isbn].d = '';
  }

  $('#bookdata').html( '<div><h3>' + spotdata.id[isbn].t + '</h3><img class="popup-image" src="' + spotdata.prefix + spotdata.id[isbn].src + '" />' + spotdata.id[isbn].d + '</div>');

  // gem values i formen
  $('#isbn').val(isbn);
  $('#titel').val(spotdata.id[isbn].t);
